...
public class SaveStreamProcessor
{
    ...
    public static async Task<Stream> DecompressSaveToStreamAsync(string filePath, CancellationToken cancellationToken = default)
    {
        ...
        Debug.WriteLine($"[ArtifactX-CORE-LOOP] Starting stream read sequence. Total file size: {fileStream.Length} bytes.");
        ...
        while (fileStream.Position < fileStream.Length)
        {
            ...
            if (bytesRead < 16)
            {
                Debug.WriteLine($"[ArtifactX-CORE-LOOP] Stream reached trailing bytes space ({bytesRead} bytes left). End of blocks.");
                break;
            }
            ...
            Debug.WriteLine($"[ArtifactX-CORE-LOOP] Parsing block at offset {currentBlockOffset}:");
            ...
            {
                Debug.WriteLine("[ArtifactX-CORE-LOOP] Encountered zero or invalid sizing marker. Finalizing assembly stream loop.");
                break;
            }
        ...
        Debug.WriteLine($"[ArtifactX-CORE-LOOP] Stream extraction complete. Final raw JSON memory map footprint: {decompressedOutputMemory.Length} bytes.");
