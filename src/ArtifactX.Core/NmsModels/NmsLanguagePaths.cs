...
///
/// SHARED across all 4 races in ONE array, not one array per race - each
/// entry's own id prefix identifies which race's word it is:
///   TRA_ = Gek ("Traders"), WAR_ = Vy'keen ("Warriors"),
///   EXP_ = Korvax ("Explorers"), BUI_ = Autophage.
/// A per-race page must read/write this SAME path, filtering only its own
/// prefix for display while preserving every other race's entries untouched
...
/// words per race exist in the full vocabulary; a given save's actual known
/// subset is normally a small fraction of that (e.g. 21-140 in a real
/// mid-progress save).
/// </summary>
