...
if (args.Length > 0 && args[0].Equals("multitool", StringComparison.OrdinalIgnoreCase))
{
    ...
    {
        ConsoleStyle.Error("No valid NMS installation path configured yet - run the normal build once first.");
        return;
    }
    ...
    {
        IReadOnlyList<NMS.Tools.DataCataloger.Models.PakEntry> entries;
        try
        {
...
if (args.Length > 1 && args[0].Equals("grep", StringComparison.OrdinalIgnoreCase))
{
    ...
    {
        NMS.Tools.DataCataloger.Models.PakHeader header;
        IReadOnlyList<NMS.Tools.DataCataloger.Models.PakEntry> entries;
        try
        {
...
// field (or NMSTemplate string-wrapper, e.g. NMSString0x10) containing the needle.
...
static bool ContainsStringDeep(object? obj, string needle, int depth, HashSet<object> visited)
{
    ...
    string? wrapped = NMS.Tools.DataCataloger.Services.ReflectionUtil.ExtractString(obj);
    if (wrapped != null)
...
if (args.Length > 1 && args[0].Equals("inspect", StringComparison.OrdinalIgnoreCase))
{
    ...
    using var db = new NMS.Tools.DataCataloger.Data.CatalogDbContext(sourcePath);
